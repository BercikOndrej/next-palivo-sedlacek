...
const PriceListSection = () => {
  ...
  const data: Wood[] = parse(fileContent, {
    delimiter: ';',
    columns: (headers) => headers.map((h) => HEADER_MAP[h] ?? h),
  ...
  return (
    <section
      className='w-full bg-bg-color py-8 p-4 md:p-8'
      id='section--price-list'
    >
      <div className='w-full mx-auto max-w-6xl flex flex-col gap-8'>
        <div>
          <SectionTitle title='Ceník' />
          <h3 className='text-primary font-bold text-2xl'>
            Suchého palivového dříví
          </h3>
          <p>
            Naše dřevo je v suchém stavu k okamžitému použití. (Do vyprodání
            skladových zásob.)
          </p>
        </div>
        <PriceList data={data} />
